...
public class ProductService {
    ...
    public Page<Product> getProductPage(int pageNum,int pageSize,String condition){
        ...
        Page<Product> productPage=Product.dao.paginate(pageNum,pageSize,"select * ","from tb_product where active=false and"+ condition+" order by sort_num desc,id desc");
...
*/
    ...
    public List<Product> getProductList() {
        List<Product> productList= Product.dao.find("select * from tb_product where active=false and isMarketable=1 order by sort_num desc,id desc");
...
*/
    ...
    public List<Product> getProductNoPromotionList(Promotion promotion) {
        String sql="SELECT *,tb_product.id as productId from tb_product where active=false and isMarketable=1 and id not in\n" +
    ...
    public int getCategoryProductCount(int category){
        return Product.dao.find("SELECT id from tb_product WHERE active=false and productCategoryId=?",category).size();
    }
    ...
    public ProductPromotion getProductPromotion(int productId) {
        return ProductPromotion.dao.findFirst("SELECT tb_product_promotion.* ,tb_promotion.name,tb_promotion.reduceMoney as promotion_reduceMoney,tb_promotion.tag,tb_promotion.type,tb_promotion.startDate,tb_promotion.endDate,tb_promotion.lastSpendMoney from tb_product_promotion LEFT JOIN tb_promotion on tb_product_promotion.promotionId = tb_promotion.id  where productId=? and startDate<? and endDate> ?",productId,new Date(),new Date());
    }
    ...
    public List<Coupon> getNoLCouponList(int userId){
        List<Coupon> couponList=Coupon.dao.find(" SELECT * from tb_coupon WHERE id not in (SELECT couponId from tb_user_coupon where userId=?)",userId);
        return couponList;
    }
    public Coupon getCouponByUserId(int userId,float totalPrice){
        return Coupon.dao.findFirst("SELECT tb_coupon.*,tb_user_coupon.id  as user_coupon_id from tb_coupon LEFT JOIN tb_user_coupon on tb_user_coupon.couponId=tb_coupon.id where tb_user_coupon.`status`=false and expiryDate>? and leastSpendMoney<=? and userId=? ORDER BY money desc",new Date(),totalPrice,userId);
    }
    public UserCoupon getUserCoupon(int userId,int  couponId){
       return UserCoupon.dao.findFirst("SELECT * from tb_user_coupon where userId=? and couponId=?",userId,couponId);
    }
    public UserCoupon lingquCoupon(int userId,int  couponId){
