...
import com.gz.common.FieldService;
import com.gz.common.LanmuService;
...
import com.gz.common.model.RoleLanmu;
...
public class IndexController extends Controller{
    public void login(){
        ...
        if(user!=null&& MD5.validPassword(password,user.getPassword())){
            user.put("avatar_tmp",Constant.FILE_PATH+user.getAvatar());
            setSessionAttr("user",user);
            user.setPassword("");
            user.put("role",UserService.getService().getRole(user.getId().toString()));
            user.put("token","mmmmmmmmmmmmmmmmy");
    ...
    public void getMenu(){
        int  moduleId = getParaToInt("moduleId",0);
        renderJson(Response.responseJson(0,"成功",LanmuService.getService().getMenu(1,moduleId)));
    }

    public void getAllLanmu(){
        int  moduleId = getParaToInt("moduleId",0);
        renderJson(Response.responseJson(0,"成功",LanmuService.getService().getAllLanmu(1,moduleId)));
    }
